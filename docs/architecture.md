...
| 向量库 | pgvector (HNSW, cosine) — Postgres 扩展, 与关系库同实例 |
| 词法检索 | ParadeDB pg_search (BM25) — 同一 Postgres |
| 关系库 | PostgreSQL (asyncpg) — 事故台账 + 向量/BM25 知识库 |
| 队列 | Redis 7 Streams |
| 工具协议 | MCP (Model Context Protocol) |
| 容器化 | Docker Compose (ParadeDB/Postgres + Redis) |

# 知识库多源摄取与增量同步 — 设计文档 (v2)

> 状态: Draft · 日期: 2026-06-22 · 范围: 飞书数据源 + MinerU 官方 API 解析 + pgvector 增量入库
>
> v2 修订 (执行计划审查后): ①删除已废弃的 BM25 手动刷新 (现为 pg_search 自动维护) ②同步改为
> 「先嵌入、再单事务 delete+insert」避免数据丢失 ③content_hash 改对 markdown 取，docx 字节哈希不可靠
> ④飞书节点类型白名单 ⑤存量 chunk 回填 ⑥基线锁定为「pgvector 迁移完成后」。

---

## 0. 前置依赖与基线 (重要)

**本计划的基线 = `app/core/pg_vector_store.py` 的 pgvector 迁移完成并合并之后。**
该文件当前正被另一会话并发修改 (向量库 Milvus→pgvector + BM25→pg_search 迁移进行中)，
函数与行号仍在变动。**在迁移收尾前不要启动 P0**，否则改动必然冲突。

迁移完成后需确认的既有事实 (本设计已据此修订):
- BM25 已是 **ParadeDB pg_search** 索引 (`pg_vector_store.py` 建在 `kb_chunks` 上)，
  **随 INSERT/DELETE 自动维护，无需任何手动刷新** (`document_service.py` 注释已明确)。
- 现有删除仍按 `source` (`delete_by_source`)；本计划新增按 `doc_id` 的路径。
- `add_documents` 内部「先算 embedding (网络调用，可能失败) 再 INSERT」，且自带连接。
...
### 4.5 调度 — 复用现有 queue-worker

复用 `app/diagnosis_worker.py` 的同款 worker/队列模式，不引入 Celery/Airflow 等新组件。逻辑任务：

| 任务 | 触发 | 动作 |
|---|---|---|
| `kb.source.sync` | 定时 (interval，默认 30min) + 手动 API | 执行 4.3 整套同步流程 |

> 本期文档量小，单 `kb.source.sync` 任务内**串行**处理各文档即可 (受飞书/MinerU QPS 限制，串行更稳)。
> 量大后再拆 `kb.doc.ingest` 子任务并发 (用 `asyncio.Semaphore` 限并发)，接口已为此预留。
> 定时器: 进程内 `asyncio` 周期任务或 APScheduler，启动时注册；并发安全靠 `kb_sync_run` + advisory lock 防同源重入。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/v1/documents/upload` | 扩展支持 pdf/doc/ppt/图片/html → MinerU；md/txt 直通。MinerU 失败返回 **503 "解析服务不可用，请稍后重试"**，不入库。 |
| POST | `/api/v1/kb/sources` | 注册飞书源 (写 `kb_source`)。 |
| GET | `/api/v1/kb/sources` | 列出源及 `last_sync_at`。 |
| POST | `/api/v1/kb/sources/{id}/sync` | 手动触发一次同步 (入队 `kb.source.sync`)。 |
| GET | `/api/v1/kb/documents` | 列出 `kb_document` (含 status/last_error)，替代/补充现有按 source 聚合。 |
...
mineru_timeout_sec: float = 300.0
...
> **前置 (B2)**: 全部阶段以「pgvector 迁移合并完成」为基线，详见第 0 节。迁移收尾前不启动 P0。

| 阶段 | 内容 | 产出/验收 |
|---|---|---|
| **P0 地基** | `kb_chunks` 加 `doc_id` + `delete_by_doc_id` + **`replace_doc` 原子替换 (C1)**；**存量清库重灌或回填 doc_id (G2)**；建 `kb_source/kb_document/kb_sync_run` 表；现有 md/txt 上传迁到分配 doc_id 的新模型 | 现有功能不回归，删除/列表走 doc_id，旧 chunk 无空 doc_id |
| **P1 Parser** | `MinerUParser` (file-urls→PUT→轮询→取 full.md，`need_ocr`/大小预检)；上传 API 扩格式，二进制走 MinerU，失败 503 | 能上传一个 pdf，落库可检索；MinerU 宕时返回 503 |
| **P2 Connector** | `FeishuConnector` (token 缓存 + list_changes + **类型白名单 G1** + export→docx + 下载)；注册源 API | 能列出飞书 Wiki 节点并取到一篇 docx 字节，非支持类型被跳过记日志 |
| **P3 Sync Engine** | `kb_sync_service` 全量+增量+删除传播+版本粗筛/md哈希免重嵌+重试；手动 sync API | 跑通一次飞书同步，重复跑命中 skip，改一篇能增量更新，删一篇能传播 |
| **P4 调度** | 接入 queue-worker + 定时器；`kb_sync_run` 审计 | 定时自动同步，审计可查 |

每阶段可独立上线；P0 不依赖任何外部源即可先行 (但需等 B2 基线)。
...
| 项 | 说明 | 应对 |
|---|---|---|
| 飞书应用授权 | 应用须被加为目标知识库「协作者」否则读空 | 接入前在飞书后台确认；文档化操作步骤 |
...
| 动作 | 文件 |
|---|---|
| 改 | `app/core/pg_vector_store.py` (doc_id 列 + delete_by_doc_id + replace_doc 原子替换) |
| 改 | `app/services/document_service.py` (扩格式 + 二进制走 MinerU) |
| 改 | `app/api/v1/documents.py` (上传扩格式 + 503) |
| 改 | `app/config.py` / `.env.example` (新增配置项) |
| 新增 | `app/core/parsers/base.py`, `app/core/parsers/mineru.py` |
| 新增 | `app/core/connectors/base.py`, `app/core/connectors/feishu.py` |
| 新增 | `app/services/kb_sync_service.py` |
| 新增 | `app/core/kb_sync_schema.py` (建 3 张表) |
| 新增 | `app/api/v1/kb_sources.py` (源管理 + 手动 sync) |
| 复用 | `app/diagnosis_worker.py` 模式 (调度), `app/core/splitter.py`, `app/rag/*` (不改) |
```

...
实现极其简洁，核心只有一个 Redis 操作：

```
key = rate_limit:{scope}:{identity}:{window_bucket}
INCR key  →  count
如果 count == 1 → EXPIRE key window_sec  （首次访问设过期时间）
...
> "限流用的是固定窗口计数器——单次 Redis INCR，实现简单、一次往返、足够挡住刷接口。边界突刺（窗口交界处瞬时 2x）对运维平台可接受。三组规则分别保护手动诊断和 Webhook，fail-open 设计保证 Redis 挂了不影响核心功能。"

### 面试追问

**Q：为什么不用滑动窗口或令牌桶？**

> 固定窗口的缺点是窗口交界处有突刺（理论上一瞬间可以通过 2 倍 limit 的请求）。但对运维平台来说，这个突刺完全可以接受——又不是支付接口，多放几个请求进来最多多排几个队。滑动窗口需要 Lua 脚本或 Redis Sorted Set，复杂度上去了收益不大。需要更平滑时，接口不变，底层换成 Lua 滑动窗口即可。
...
| `worker_diagnosis` | 2 | `wait=True` | Worker 消费，满了排队等 |
...
1. **申请**：Redis INCR，超过 limit 则根据 wait 决定等还是拒
...
> "并发槽和限流是两个维度：限流管速率，并发槽管同时执行数。两个都需要，缺一不可。并发槽用 Redis INCR/DECR + TTL 实现，跨进程共享。亮点是审批等待时会 pause 释放槽，审批通过后 resume 重新获取，不浪费宝贵的执行名额。"

...
采用 **固定窗口计数器 + Redis INCR** 实现，代码在 `app/core/rate_limiter.py`：
...
| Readiness | `GET /health/ready` | Milvus 必须可用；Postgres/Redis 在开启 incident pipeline 时必须可用；MCP 可选 |
...
知道这个问题。固定窗口在窗口交界处确实可能放行 2 倍的请求。滑动窗口或令牌桶更精确，但实现复杂度也更高。当前场景下这个突刺可以接受——限流的目的是防刷爆而不是精确控速，突刺最多就是多放行一个窗口的请求量，后面有队列和执行槽兜底。如果上生产发现突刺是实际问题，可以升级到滑动窗口 + Redis ZSET 方案。
...
Liveness 只看进程是否存活。Readiness 检查核心依赖（Milvus 必须、Postgres/Redis 按配置）。Redis 临时不可用时，Liveness 仍然 200（进程没挂），Readiness 返回 503。K8s 看到 503 只停止发送新请求，不会杀进程重启——杀进程反而加重故障。

### 反思与改进

**面试官：API 设计这块如果重来，你会改什么？**

两个。一是限流器升级到滑动窗口——固定窗口突刺问题虽不严重，但面试被问到确实尴尬，滑动窗口用 Redis ZSET 实现也不复杂。二是 SSE 诊断加断点续传——当前连接断了诊断就丢了，应该在 Redis 里缓存中间状态，客户端重连后从上次断点继续接收。
